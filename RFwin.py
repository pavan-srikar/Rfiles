...
# 4. Converted to a Windows-compatible version.
...
def start(update, context):
    ...
    kernel_version = os.popen('ver').read().strip()
    ip_address = socket.gethostbyname(socket.gethostname())

    welcome_message = f"Welcome to the Windows File Access Bot, {username}! :)\n\n"
    welcome_message += f"System Details:\n"
    welcome_message += f"  Username: {username}\n"
    welcome_message += f"  Windows Version: {kernel_version}\n"
...
def get_dir_zip(update, context):
    ...
    try:
        ...
        os.system(f'powershell Compress-Archive -Path "{directory}" -DestinationPath "{zip_file_name}"')
